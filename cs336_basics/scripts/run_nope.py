...
# RUN_SHORT_SWEEP = True
# RUN_FULL_SWEEP = False
# FULL_LRS = [1e-3, 2e-3, 3e-3]
...
def run_one(alpha_max: float, total_steps: int, run_name: str):
    ...
    training_together(
        ...
        checkpoint_path=f"runs/nope/{run_name}/checkpoint.pt",
...
def main():
    ...
    if RUN_FULL_SWEEP:
        for lr in FULL_LRS:
            run_name = f"full_lr_{lr:.0e}_{FULL_TOTAL_STEPS}steps"
            run_one(lr, FULL_TOTAL_STEPS, run_name)
